...
exclude_nodes = ['LGBT', 'LGBTQ', 'LGBTQ+', 'Queer', 'Full Article', 'RT.com', 'Breitbart', 'Forbes', 'The Advocate', 'And The', 'NBC', 'BBC']
...
news_title_dict = {t[2]: t[0] for t in news.itertuples(index = False)}


def concat_news_link(news_id):
    news = ""
    for iid in news_id:
        news += news_link_dict[iid] + ", "
    return news[:-2]

entity_info['news_links'] = entity_info.apply(lambda x: concat_news_link(x['news_id']), axis = 1)

def concat_news_title(news_id):
    news = ""
    for iid in news_id:
        news += news_title_dict[iid] + "; "
    return news[:-2]

entity_info['news_title'] = entity_info.apply(lambda x: concat_news_title(x['news_id']), axis = 1)
...
def concat_news_link(repre_news):
    news = ""
    for iid in repre_news.split(','):
        news += news_link_dict[int(iid)] + ", "
    return news[:-2]

cluster['news_links'] = cluster.apply(lambda x: concat_news_link(x['repre_news']), axis = 1)

def concat_news_title(repre_news):
    news = ""
    for iid in repre_news.split(','):
        news += news_title_dict[int(iid)] + "; "
    return news[:-2]

cluster['news_titles'] = cluster.apply(lambda x: concat_news_title(x['repre_news']), axis = 1)
